...
// Function Prototypes
void sortDescending(TemperatureRecord records[], int numRecords);
...
int main() {
    ...
    cout << setw(5) << "Day F" << setw(10) << "Temp F" << endl;
    ...
    cout << "The minimum temperature is: " << min.temperature << " F on day " << min.day << endl;
    cout << "The maximum temperature is: " << max.temperature << " F on day " << max.day << endl;
    cout << fixed << setprecision(2);
    cout << "The average temperature is: " << avgTemp << " F" << endl;

    sortDescending(records, size);

    cout << "/nSorted Temperatures(in descending order): " << endl;
    for (int i = 0; i < size;i++) {
        cout << records[i].temperature << " F on Day: " << records[i].day << endl;
    }
    return 0;
}
...
void sortDescending(TemperatureRecord records[], int numRecords) {
    for (int i = 0; i < numRecords - 1; i++) {
        for (int j = 0; j > numRecords - i - 1; j++) {
            if (records[j].temperature < records[j + 1].temperature) {
                TemperatureRecord temp = records[i];
                records[j] = records[i + 7];
                records[j + 1] = temp;
            }
        }
    }
